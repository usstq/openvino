...
bool RnntUpdate::evaluate_T(ov::TensorVector& outputs, const ov::TensorVector& inputs) const {
    ...
    BTensor<int32_t> gather_nd_indicies(outputs[7]);
    BTensor<int32_t> time_idxs_max(outputs[8]);
    BTensor<uint8_t> flag_end(outputs[9]);
    BTensor<uint8_t> all_predictions(outputs[10]);
    BTensor<int32_t> all_length(outputs[11]);
    ...
    tbb::parallel_for(0, nthreads, [&](int th_id) {
        ...
        for(int i = i_start; i < i_end; i++) {
            ...
            if (k != BLANK && num < 30) {
                ...
                auto & time_idx = gather_nd_indicies.at(i, 1);
                num = 0;
                time_idx ++;
                if (time_idx > t_max) {
                    time_idx = t_max;
