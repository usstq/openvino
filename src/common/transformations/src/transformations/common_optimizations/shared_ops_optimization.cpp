...
bool inputs_from_same_source_or_equal_constants(const std::shared_ptr<Node>& lhs, const std::shared_ptr<Node>& rhs, int shared_input_idx) {
    if (lhs->get_input_size() != rhs->get_input_size())
        return false;
    size_t input_size = lhs->get_input_size();
    for (size_t i = 0; i < input_size; ++i) {
        if (i == shared_input_idx)
            continue;
...
struct NodeInfo {
    ...
    NodeInfo(const std::shared_ptr<Node>& node, int shared_input_idx) : node(node), shared_input_idx(shared_input_idx) {}
    bool extract_attrs() {
...
struct SSS {
    ...
    void add(int nodes, int compares) {
        std::pair<int, int> v{nodes, compares};
        if (data.count(v)) {
            data[v] ++;
        } else {
            data[v] = 1;
    ...
    ~SSS() {
        for(auto& v : data) {
            std::cout << v.first.first << ":" << v.first.second << " x " << v.second << std::endl;
        }
    }
};
...
bool shared_node_optimization(const shared_ptr<Model>& model) {
        ...
        for (auto& output : op->outputs()) {
            ...
            for (auto& item : type_to_node) {
                ...
                std::sort(shared_nodes.begin(),
                          shared_nodes.end(),
                          [&index_map](const NodeInfo& a, const NodeInfo& b) {
    ...
    if (is_subgraph) {
        ov::pass::PrintModel ppp("ppp.cpp");
        ppp.run_on_model(model);
        std::cout << order.size() << ":" << total_hit << "/" << total_output << ": " << total_visits << ":" << total_compares << std::endl;
        abort();
    }
    sss.add(order.size(), total_compares);
    return rewritten;
}
...
bool pass::SharedOpOptimization::run_on_model(const shared_ptr<Model>& model) {
    ...
    ov::pass::PrintModel ppp("main.cpp");
    ppp.run_on_model(model);
