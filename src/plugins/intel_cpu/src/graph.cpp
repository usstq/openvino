...
void Graph::EnforceBF16() {
    ...
    for (const auto& node : graphNodes) {
        ...
        // Input from const shouldn't expose as bf16
        if (node->getType() == Type::Input && node->isConstant())
            continue;

        // Skip Input & Output for main graph
        ...
        for (size_t i = 0; i < node->getOriginalInputsNumber(); i++) {
            ...
            if (parent->getType() == Type::Input && parent->isConstant()) {
                ...
                if (node->getType() != Type::Concatenation && node->getType() != Type::Gather)
                    doEnforceBF16 = false;
            }

            // exclude Eltwise after Input since it supports conversion from other precisions into BF16
            if (parent->getType() == Type::Input && node->getType() == Type::Eltwise) {
                doEnforceBF16 = false;
            }

            if (doEnforceBF16 && node->getOriginalInputPrecisionAtPort(i) == Precision::FP32)
                node->setOriginalInputPrecisionAtPort(i, Precision::BF16);
        }

        // always prefer BF16 for output precision
