...
ov::intel_cpu::RoPEFusionChatGLM::RoPEFusionChatGLM(int split_output_id) {
    ...
    matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        std::cout << "RoPEFusionChatGLM: " << split_output_id << " head_cnt=" << validator["head_cnt"]
                  << " from " << pattern_map.at(cur_key) << std::endl;

        // input  f32 [length, batch_size, (hidden_states_q + hidden_states_k + hidden_states_v)]
        // output f32 [length, batch_size, head_cnt, hidden_states_k]
        // sin_cos (has been gathered with positions)
        //        f32 [length, batch_size, ndims//2, 2]
