...
ov::intel_cpu::CausalMaskPreprocess::CausalMaskPreprocess() {
    MATCHER_SCOPE(CausalMaskPreprocess);

    if (std::getenv("NOCCC"))
        return;

    auto const_triu = makePattern<opset1::Constant>({}, {});
    // size_Gather  i32[1]  repeat (batchsize, 1, 1, 1)
    // auto attention_mask = makeOP<opset1::Parameter>({}, {{"shape", [?,?]}, {"element_type", "i64"}});   //
    ...
    auto ListConstruct_Concat =
        makePattern<opset1::Concat>({ShapeOf_41610, {1}, {1}, {1}}, {{"axis", 0}});  //  tensor_array<i32[4]>
    auto repeat_Tile =
        makePattern<opset1::Tile>({const_triu, ListConstruct_Concat});  //  tensor_array<u8[?,1,8192,8192]>
    auto to_Convert =
        makePattern<opset1::Convert>({repeat_Tile}, {{"destination_type", "f32"}});  //  tensor_array<f32[?,1,8192,8192]>
    auto Constant_107277 = makeConst(element::f32,
                                     ov::Shape({
                                         1,
                                         1,
                                         1,
                                     ...
                                     {-FLT_MAX});
    auto mul_Multiply_1 = makePattern<opset1::Multiply>({to_Convert, Constant_107277},
                                                   {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,1,8192,8192]>
    auto SliceAssign_201_Reshape_0 =
        makePattern<opset1::Reshape>({mul_Multiply_1, {-1}}, {{"special_zero", false}});  //  tensor_array<f32[?]>
    auto SliceAssign_201_ShapeOf = makePattern<opset1::ShapeOf>({mul_Multiply_1});        //  tensor_array<i32[4]>
    auto SliceAssign_201_ReduceProd =
        makePattern<opset1::ReduceProd>({SliceAssign_201_ShapeOf, 0}, {{"keep_dims", false}});  //  tensor_array<i32[]>
    auto SliceAssign_201_Range =
        makePattern<opset4::Range>({0, SliceAssign_201_ReduceProd, 1}, {{"output_type", "i32"}});  //  tensor_array<i32[?]>
    auto SliceAssign_201_Reshape =
        makePattern<opset1::Reshape>({SliceAssign_201_Range, {-1, 1, max_seq_len, max_seq_len}},
                                {{"special_zero", true}});  //  tensor_array<i32[?,1,8192,8192]>
    auto Convert_48852 =
        makePattern<opset1::Convert>({attention_mask}, {{"destination_type", "i32"}});  //  tensor_array<i32[?,?]>
    auto ShapeOf_49034 = makePattern<opset1::ShapeOf>({Convert_48852});                 //  tensor_array<i32[2]>
    auto Gather_41642 = makePattern<opset8::Gather>({ShapeOf_49034, {1}, 0}, {{"batch_dims", 0}});  //  tensor_array<i32[1]>
    auto ScatterUpdate_93502 =
        makePattern<opset3::ScatterUpdate>({{0, 0, 0, 0}, {3}, Gather_41642, {0}});  //  tensor_array<i32[4]>
    auto SliceAssign_201_Slice =
        makePattern<opset1::StridedSlice>({SliceAssign_201_Reshape, {0, 0, 0, 0}, ScatterUpdate_93502, {1, 1, 1, 1}},
                                     {{"begin_mask", {1, 1, 1, 0}},
                                      {"end_mask", {1, 1, 1, 0}},
                                      {"new_axis_mask", {}},
                                      {"shrink_axis_mask", {}},
                                      {"ellipsis_mask", {}}});  //  tensor_array<i32[?,1,8192,..8192]>
    auto SliceAssign_201_Reshape_1 = makePattern<opset1::Reshape>({SliceAssign_201_Slice, {-1, 1}},
                                                             {{"special_zero", false}});  //  tensor_array<i32[?,1]>
    auto causal_mask_boolean_1 =
        makePattern<opset1::StridedSlice>({mul_Multiply_1, {0, 0, 0, 0}, ScatterUpdate_93502, {1, 1, 1, 1}},
                                     {{"begin_mask", {1, 1, 1, 0}},
                                      {"end_mask", {1, 1, 1, 0}},
                                      {"new_axis_mask", {}},
                                      {"shrink_axis_mask", {}},
                                      {"ellipsis_mask", {}}});  //  tensor_array<f32[?,1,8192,..8192]>
    auto Constant_107278 = makeConst(element::f32,
                                     ov::Shape({
                                         1,
                                         1,
                                         1,
                                     ...
                                     {0.000000f});
    auto eq_Equal = makePattern<opset1::Equal>({causal_mask_boolean_1, Constant_107278},
                                          {{"auto_broadcast", "numpy"}});  //  tensor_array<u8[?,1,8192,..8192]>
    auto unsqueeze_Unsqueeze_1 = makePattern<opset1::Unsqueeze>({Convert_48852, {1, 2}});  //  tensor_array<i32[?,1,1,?]>
    auto eq_Convert =
        makePattern<opset1::Convert>({unsqueeze_Unsqueeze_1}, {{"destination_type", "f32"}});  //  tensor_array<f32[?,1,1,?]>
    auto Constant_107279 = makeConst(element::f32,
                                     ov::Shape({
                                         1,
                                         1,
                                         1,
                                     ...
                                     {0.000000f});
    auto eq_Equal_1 = makePattern<opset1::Equal>({eq_Convert, Constant_107279},
                                            {{"auto_broadcast", "numpy"}});  //  tensor_array<u8[?,1,1,?]>
    auto mul_LogicalAnd = makePattern<opset1::LogicalAnd>({eq_Equal, eq_Equal_1},
                                                     {{"auto_broadcast", "numpy"}});  //  tensor_array<u8[?,1,8192,?]>
    auto masked_fill_Select = makePattern<opset1::Select>({mul_LogicalAnd, -FLT_MAX, causal_mask_boolean_1},
                                                     {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,1,8192,?]>
    auto copy__ShapeOf = makePattern<opset1::ShapeOf>({causal_mask_boolean_1});          //  tensor_array<i32[4]>
    auto Constant_47319 = makeConst(element::u8, ov::Shape({}), 0);
    auto copy__Broadcast = makePattern<opset1::Broadcast>({masked_fill_Select, copy__ShapeOf, Constant_47319},
                                                     {{"mode", "numpy"}});  //  tensor_array<f32[?,1,8192,..8192]>
    auto SliceAssign_201_Reshape_2 =
        makePattern<opset1::Reshape>({copy__Broadcast, {-1}}, {{"special_zero", false}});  //  tensor_array<f32[?]>
    auto SliceAssign_201_ScatterNDUpdate = makePattern<opset4::ScatterNDUpdate>(
    ...
    auto SliceAssign_201_Reshape_3 =
        makePattern<opset1::Reshape>({SliceAssign_201_ScatterNDUpdate, {-1, 1, max_seq_len, max_seq_len}},
                                {{"special_zero", true}});  //  tensor_array<f32[?,1,8192,8192]>

    // Add_41653     : kvLen
    // index_Convert : cache_positions
    auto ScatterUpdate_93554 =
        makePattern<opset3::ScatterUpdate>({{0, 0, 0, 0}, {3}, kvLen, {0}});  //  tensor_array<i32[4]>
    auto slice_Slice_14 =
        makePattern<opset1::StridedSlice>({SliceAssign_201_Reshape_3, {0, 0, 0, 0}, ScatterUpdate_93554, {1, 1, 1, 1}},
                                     {{"begin_mask", {1, 1, 1, 0}},
                                      {"end_mask", {1, 1, 1, 0}},
                                      {"new_axis_mask", {}},
                                      {"shrink_axis_mask", {}},
                                      {"ellipsis_mask", {}}});  //  tensor_array<f32[?,1,8192,..8192]>
    auto index_Gather = makePattern<opset8::Gather>({slice_Slice_14, cache_positions, 2},
                                               {{"batch_dims", 0}}, nullptr, "XXXXX");  //  tensor_array<f32[?,1,?,..8192]>
    auto result = index_Gather;

    std::shared_ptr<opset1::Constant> global_triu;
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) mutable {
        const auto& pattern_map = m.get_pattern_value_map();
        auto root = m.get_match_root();
        PatternValidator validator(m);
        ...
        auto triu =
            std::dynamic_pointer_cast<opset1::Constant>(pattern_map.find(const_triu)->second.get_node_shared_ptr());
        ...
        auto replacement = std::make_shared<VNode>(inputs, config);
        ov::replace_node(root, replacement);
