    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (pattern_map.count(gate_proj_weight_compressed) > 0 && pattern_map.count(up_proj_weight_compressed) > 0 &&
            ...
            down_proj_w = pattern_map.at(down_proj_weight_compressed);
                   ...
                   pattern_map.count(down_proj_weight_i8) > 0 &&
                   pattern_map.count(gate_proj_weight_scales_per_OC) > 0 &&
                   pattern_map.count(up_proj_weight_scales_per_OC) > 0 &&
                   pattern_map.count(down_proj_weight_scales_per_OC) > 0) {
            is_quantized_int8 = true;
        ...
        std::cout << "MLPFusion: " << __LINE__ << "  " << root->get_friendly_name()
                  << "  is_is_quantized_int8=" << is_quantized_int8 << std::endl;
